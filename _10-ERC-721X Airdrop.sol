...
contract AIRDROP is ERC721X, EIP2981, Ownable {
   using Strings for uint256;
    ...
    constructor(string memory _initNotRevealedUri, string memory _initBaseURI, address _royaltyRecipient, uint256 _royaltyAmount) EIP2981(_royaltyRecipient, _royaltyAmount) ERC721X("AIRDROP", "AIR") Ownable() {
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory)
  {
    ...
    function mint(address account)
        ...
        onlyOwner
    {
        require(nextId <= maxTotalSupply);
            _safeMint(account, false);
           }
